...
Observation:
> There are more than 2 million parameters in the `base_model_layer`! These are way more than the no. of parameters we see in model 1 back in the model summary. However, there are only 2 thousands trainable params in this model training since we do not need all the params for our task when we use the base model. 
